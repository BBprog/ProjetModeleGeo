...
void Courbures::set_K_colors()
{
    ...
    for (v_it = _mesh.vertices_begin(); v_it != _mesh.vertices_end(); ++v_it)
    {
        tmp = _mesh.property(vprop_K, *v_it);
		// Carte de couleur de tmp (pour les valeurs comprises entre
		// m-sigma / m+sigma
		
        // TODO
		
        tmp_color = OpenMesh::Vec3uc(0,0,0) ;
        if (tmp != 0)
        {
            if (tmp > 0)
                tmp_color = OpenMesh::Vec3uc(25+220*tmp/sigma,0,0);
            else
                tmp_color = OpenMesh::Vec3uc(0,0,25+220*(-tmp)/sigma);
        }
